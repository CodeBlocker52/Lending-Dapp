...
contract LendingBorrowing {
    ...
    function getLoan(uint256 _collateralAmount, uint256 _loanAmount) public {
        require(
            loanTokenContract.balanceOf(loanTokenAddress) >= _loanAmount * dec,
            "Insufficient liquidity"
        );
        loanTokenContract.transferTokens(
            address(loanTokenAddress),
            msg.sender,
            _loanAmount
        );
        collateralTokenContract.transferTokens(
            msg.sender,
    ...
    function repayLoan() public {
        ...
        uint256 repaymentAmount = l.loanAmount +
